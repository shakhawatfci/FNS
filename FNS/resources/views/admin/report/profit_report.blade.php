 				...
 				<table class="table table-bordered table-hover table-condensed">
                  ...
                  @foreach($sell as $value)
 	               <tr>
 	                ...
 	                <td class="text-right">{{ $quantity = $value->total_sold_quantity  }}</td>

 	                <td class="text-right">{{ $value->total_sold_price  }}</td>
 	                <td class="text-right">{{ $value->total_buying_price }}</td>
 	                <td class="text-right">{{ $value->total_sold_price - $value->total_buying_price  }}</td>
                  ...
                  $total_sold_price  += $value->total_sold_price;
 				  ...
 				  <tr style="color: #fff;background-color: #000;font-weight: bold;">
 				  	 ...
 				  	 <td class="text-right">{{ number_format($total_sold_price,2) }}</td>
 				  	 <td class="text-right">{{ number_format($total_buying_price,2) }}</td> 

 				  	 <td class="text-right">{{ number_format(($total_sold_price-$total_buying_price),2) }}</td>
 				  </tr>
 				</table>
 			</div>
 			</div>
